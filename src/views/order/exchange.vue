    ...
    <a-card :bordered="false">
        ...
        <a-col>
          <a-form layout="inline" :form="form" @submit="handleSubmit">
            <a-form-item label="换货单号">
              <a-input style="width: 140px" v-decorator="['name']" placeholder="换货单号" allowClear />
            </a-form-item>
            <a-form-item label="订单编号">
              <a-input style="width: 140px" v-decorator="['username']" placeholder="订单编号" allowClear />
            </a-form-item>
      ...
      <a-table :columns="columns" :dataSource="data" :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" :rowKey="(v,k) => k" :pagination="pagination" @change="tableChange">
        <span slot="action" slot-scope="text, record">
          <router-link :to="'/order/exchangede?id='+record.number">详情</router-link>
...
import { exchangeColumns, exchangeData } from './config';
export default {
  name: 'orderExchange',
  components: { PageView },
  data() {
    return {
      ...
      form: this.$form.createForm(this, { name: 'orderExchange' })
    };
  },
  methods: {
    handleSubmit(e) {
